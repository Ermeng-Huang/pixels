%drawTotalProbes (raw code from xd)
homedir='D:\histology\dualtask';
...
ProbeColors = .75*[1.3 1.3 1.3; 1 .75 0;  .3 1 1; .4 .6 .2; 1 .35 .65; .7 .7 .9; .65 .4 .25; .7 .95 .3; .7 0 0; .6 0 .7; 1 .6 0; .1 .1 .1;.3 .3 .3;.5 .5 .5;.7 .7 .7;.9 .9 .9;.1 .9 .9; .1 .7 .7;.1 .5 .5;1 1 1; 1 .75 0]; 
...
pointLists = dir(fullfile(homedir,'M*\processed\probe_pointselectrode_1.mat'));
if length(pointLists) > 11
    ProbeColors = .75*distinguishable_colors(length(pointLists),'w');
end

probe_lengths = cell(length(pointLists),2);
%% define the cell for probelengths here 
fl=dir(fullfile(homedir,'M*\results\HistLength.mat'));
for f=1:length(fl)
    rootpath=fl(f,1).folder;
    probe_lengths{f,1}=rootpath(end-10:end-8); %mouseID
    Probe_length=load(fullfile(rootpath,'HistLength.mat'));
    probe_lengths{f,2}=Probe_length.Probe_length;
end
%%

for i = 1:length(pointLists)
    currPointListName = load(fullfile(pointLists(i,1).folder,pointLists(i,1).name));
    for j = 1:size(currPointListName.pointList.pointList,1)
        ...
        currLengths = probe_lengths{i,2}/1000;
...
saveas(fwireframe,fullfile(homedir,'dualtask_Probe.fig'),'fig')
exportgraphics(fwireframe,fullfile(homedir,'dualtask_Probe.pdf'))

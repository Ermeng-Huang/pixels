...
save(fullfile(homedir,sprintf('sust_trans_%d_%d_pair.mat',ubegin,uend)),'sust_trans','ubegin','uend','i','path')
...
trials1=trials(:,5)~=trials(:,6)&all(trials(:,end-1:end)==1,2); % pair
trials2=trials(:,5)==trials(:,6)&all(trials(:,end-1:end)==1,2); % no-pair

...
if IsParallel
    nPool=2;
    parpool(nPool)
    count=size(dir(fullfile(homedir,'DataSum','*','*','FR_All_250ms.hdf5')),1);
    batchLen=ceil(count/nPool);
    batchStart=1:batchLen:count;
    
    futures=parallel.FevalFuture.empty(0,nPool);
    for batchIdx=1:size(batchStart,2)
        batchEnd=batchStart(batchIdx)+batchLen-1;
        if batchEnd>count
            batchEnd=count;
        end
        futures(batchIdx) = parfeval(@onebatch,0,batchStart(batchIdx),batchEnd,homedir);
    end
    ...
    f2=ls('sust_trans_*_*.mat');
    sums6=[];
    
    for i=1:size(f2,1)
        fstr=load(strtrim(f2(i,:)));
        if fstr.i~=fstr.uend
            fprintf('Error processing file');
        else
            sums6=[sums6;fstr.sust_trans];
            
        end
    end
    h5create('sust_trans_AIopto_1-298_0323.hdf5','/transient6',size(sums6),'Datatype','double');
    h5write('sust_trans_AIopto_1-298_0323.hdf5','/transient6',sums6);
else
    count=size(dir(fullfile(homedir,'DataSum','*','*','FR_All_250ms.hdf5')),1);
    onebatch(1,count,homedir)
    
end


%% Function
function onebatch(ubegin,uend,homedir)
...
save(fullfile(homedir,sprintf('sust_trans_%d_%d_distractor.mat',ubegin,uend)),'sust_trans','ubegin','uend','i','path')
end
...
for i=1:size(SU_id,1)
    ...
    [out(i,1)]=TestSecondSelectivity(FR_S1(:,1:2),FR_S2(:,1:2),'baseline',trials(trials1,:),trials(trials2,:),'anova');
    [out(i,2)]=TestSecondSelectivity(FR_S1(:,4),FR_S2(:,4),'sample',trials(trials1,:),trials(trials2,:),'anova');
%     [out(i,3:delay+2)]=TestSecondSelectivity(FR_S1(:,5:4+delay),FR_S2(:,5:4+delay),'delay',trials(trials1,:),trials(trials2,:),'anova');
    [out(i,3:2*delay+2)]=TestSecondSelectivity(FR_S1(:,5:4+delay),FR_S2(:,5:4+delay),'delay',trials(trials1,:),trials(trials2,:),'anova');
%     
%     if  out(i,2)==1
%         [~,~,transient,~,~,~]=TestSecondSelectivityChange_lite(FR_S1(:,5:10),FR_S2(:,5:10),2000,1:6,1:6);
%         if transient==0 % unclassified
%             out(i,2)=0;
%             out(i,3)=1;
%         end
%     end
    ...
    for i=1:size(FR_S1,2)
        ...
        if strcmp(method,'ranksum')
            ...
            if sum(a(:))+sum(b(:))>0
                ...
                p=anovan(data,{g1,g2},'model','interaction','display','off');
                if mean(data(g2==2))>mean(data(g2==3))
                    out(:,i)=p(2);
                    out(:,i+8)=p(3);
                elseif mean(data(g2==2))<mean(data(g2==3))
                    out(:,i)=-p(2);
                    out(:,i+8)=-p(3);
                else
                    out(:,i)=1;
...
elseif strcmp(time,'sample') || strcmp(time,'baseline')

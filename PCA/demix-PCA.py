# -*- coding: utf-8 -*-
"""
Created on Thu Jul  8 06:18:09 2021
...
class PCA_stats:
    ...
    def splitMean(self, FR, mm, std, random_type=None):
        if random_type is None:
            out = np.squeeze(np.mean((FR - mm) / std, axis=0))
        elif random_type == 'half':
            sel = list(self.kf.split(np.arange(FR.shape[1])))[0]
            out = np.squeeze(np.mean((FR[:, :,sel[0],] - mm) / std, axis=1))
        else:
            sel = np.random.choice(FR.shape[2],1)
            out = np.squeeze((FR[:, :, sel, ] - mm) / std)
        return out
    ...
    def processCTDStats(self, trial_FR, trials, welltrain_window=None, correct_resp=None, random_type=None):
        ...
        if welltrain_window:            
            ...
            NeuronSet=[]
            NeuronSet.append(np.all(np.vstack((trials[4, :] == 4, trials[5, :] == 8, trials[8,:] == 2)),axis=0 )) # S1_T1_laseron
            NeuronSet.append(np.all(np.vstack((trials[4, :] == 4, trials[5, :] == 8, trials[8, :] == -1)),axis=0 )) # S1_T1_laseroff  
            NeuronSet.append(np.all(np.vstack((trials[4, :] == 4, trials[5, :] == 4, trials[8, :] == 2)),axis=0 )) # S1_T2_laseron
            NeuronSet.append(np.all(np.vstack((trials[4, :] == 4, trials[5, :] == 4, trials[8, :] == -1)),axis=0 )) # S1_T2_laseroff
            NeuronSet.append(np.all(np.vstack((trials[4, :] == 8, trials[5, :] == 8, trials[8, :] == 2)),axis=0 )) # S2_T1_laseron
            NeuronSet.append(np.all(np.vstack((trials[4, :] == 8, trials[5, :] == 8, trials[8, :] == -1)),axis=0 )) # S2_T1_laseroff   
            NeuronSet.append(np.all(np.vstack((trials[4, :] == 8, trials[5, :] == 4, trials[8, :] == 2)),axis=0 )) # S2_T2_laseron
            NeuronSet.append(np.all(np.vstack((trials[4, :] == 8, trials[5, :] == 4, trials[8, :] == -1)),axis=0 )) # S2_T2_laseroff
        
            
            # NeuronSet.append(np.all(np.vstack((trials[4, :] == 4, trials[5, :] == 8, trials[6, :] == 1, trials[8,:] == 2)),axis=0 )) # S1_T1_D1_laseron
            # NeuronSet.append(np.all(np.vstack((trials[4, :] == 4, trials[5, :] == 8, trials[6, :] == 1, trials[8, :] == -1)),axis=0 )) # S1_ :]T1_D1_laseroff            
            # NeuronSet.append(np.all(np.vstack((trials[4, :] == 4, trials[5, :] == 8, trials[6, :] == 0, trials[8, :] == 2)),axis=0 )) # S1_T1_D2_laseron
            # NeuronSet.append(np.all(np.vstack((trials[4, :] == 4, trials[5, :] == 8, trials[6, :] ==0, trials[8, :] == -1)),axis=0 )) # S1_T1_D2_laseroff
            # NeuronSet.append(np.all(np.vstack((trials[4, :] == 4, trials[5, :] == 4, trials[6, :] == 1, trials[8, :] == 2)),axis=0 )) # S1_T2_D1_laseron
            # NeuronSet.append(np.all(np.vstack((trials[4, :] == 4, trials[5, :] == 4, trials[6, :] == 1, trials[8, :] == -1)),axis=0 )) # S1_T2_D1_laseroff
            # NeuronSet.append(np.all(np.vstack((trials[4, :] == 4, trials[5, :] == 4, trials[6, :] == 0, trials[8, :] == 2)),axis=0 )) # S1_T2_D2_laseron
            # NeuronSet.append(np.all(np.vstack((trials[4, :] == 4, trials[5, :] == 4, trials[6, :] == 0, trials[8, :] == -1)),axis=0 )) # S1_T2_D2_laseroff
            # NeuronSet.append(np.all(np.vstack((trials[4, :] == 8, trials[5, :] == 8, trials[6, :] == 1, trials[8, :] == 2)),axis=0 )) # S2_T1_D1_laseron
            # NeuronSet.append(np.all(np.vstack((trials[4, :] == 8, trials[5, :] == 8, trials[6, :] == 1, trials[8, :] == -1)),axis=0 )) # S2_T1_D1_laseroff
            # NeuronSet.append(np.all(np.vstack((trials[4, :] == 8, trials[5, :] == 8, trials[6, :] == -1, trials[8, :] == 2)),axis=0 )) # S2_T1_D2_laseron
            # NeuronSet.append(np.all(np.vstack((trials[4, :] == 8, trials[5, :] == 8, trials[6, :] ==-1, trials[8, :] == -1)),axis=0 )) # S2_T1_D2_laseroff
            # NeuronSet.append(np.all(np.vstack((trials[4, :] == 8, trials[5, :] == 4, trials[6, :] == 1, trials[8, :] == 2)),axis=0 )) # S2_T2_D1_laseron
            # NeuronSet.append(np.all(np.vstack((trials[4, :] == 8, trials[5, :] == 4, trials[6, :] == 1, trials[8, :] == -1)),axis=0 )) # S2_T2_D1_laseroff
            # NeuronSet.append(np.all(np.vstack((trials[4, :] == 8, trials[5, :] == 4, trials[6, :] == 0, trials[8, :] == 2)),axis=0 )) # S2_T2_D2_laseron
            # NeuronSet.append(np.all(np.vstack((trials[4, :] == 8, trials[5, :] == 4, trials[6, :] == 0, trials[8, :] == -1)),axis=0 )) # S2_T2_D2_laseroff
        ...
        for su_idx in range(trial_FR.shape[0]):                
            ...
            if std > 0  
                self.su_list.append(
                    {
                        "S1_T1_laseron": self.splitMean(FR_S1_laseron, su_idx, mm, std, random_type),
                        "S1_T1_laseroff": self.splitMean(FR_S2_laseron, su_idx, mm, std, random_type),
                        "S1_T2_laseron": self.splitMean(FR_S1_laseroff, su_idx, mm, std, random_type),
                        "S2_T2_laseroff": self.splitMean(FR_S2_laseroff, su_idx, mm, std, random_type),  
                        
                    }
                )
                # su_list=np.insert(su_list[:,:,0,0,0],0,np.array(self.splitMean(trial_FR[NeuronSet[1],su_idx,:], mm, std, random_type)),axis=0)
                # su_list=
                # np.append(array(self.splitMean(trial_FR[NeuronSet[1],su_idx,:], mm, std, random_type)),axis=0)
...
def get_dataset(denovo=False, to_explore=True, random_type=None):
    ...
    if denovo:        
        for path in sorted(traverse('D:\pixel-optogenetic\DataSum')):
            ...
            currStats.processCTDStats(trial_FR.transpose(2,0,1), trials, random_type=random_type)

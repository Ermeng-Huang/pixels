...
%%
f=dir(fullfile(homedir,'behavior','*.ser'));
file_id=unique(floor(h5read(fullfile(homedir,'Selectivity_1129.hdf5'),'/cluster_id')/100000));
f=f(file_id);
for i=1:size(f,1)
    [event{i},lick{i}]=ExtractEvent(util.ser2mat2(fullfile(f(i).folder,f(i).name)),crierian,lick_crierian,trial_min);
end
...
while i<=size(event,1)
    goodOff=nnz(event(i-trial_min+1:i,13)==1); %7-hit,5-correct
    lickoff=nnz(event(i-trial_min+1:i,6)==7);
